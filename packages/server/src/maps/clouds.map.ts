...
// Center hex is a locked gold hex (revealed from the start, no number token needed separately)
// Ring-1 (6 hexes) = cloud hexes hidden until explored
const centerHex: CubeCoord = CENTER;
const cloudedCoords = cubeRing(CENTER, 1);  // 6 hexes, ring-1 only
...
// 6 (cloud) + 42 (outer) = 48 productive terrain hexes; gold center is locked
const TERRAIN_POOL: Array<TerrainType | null> = [
  null, null, null, null, null, null,                                                    // 6 cloud hexes
...
export const CLOUDS_MAP: MapTemplate = {
  ...
  hexes: [
    // Gold center — locked, always visible, no terrain shuffling
    { coord: centerHex, terrain: 'gold' as TerrainType, locked: true },
    // Ring-1: 6 cloud hexes — terrain hidden until roads reveal them
    ...cloudedCoords.map((coord, i) => ({ coord, terrain: TERRAIN_POOL[i] ?? null })),
    // Ring-2: 12 desert hexes — locked barrier
    ...desertRing.map((coord) => ({ coord, terrain: 'desert' as TerrainType, locked: true })),
    // Rings 3-4: 42 outer land hexes
    ...outerRing3.map((coord, i) => ({ coord, terrain: TERRAIN_POOL[6 + i] ?? null })),
    ...outerRing4.map((coord, i) => ({ coord, terrain: TERRAIN_POOL[24 + i] ?? null })),
  ],
  cloudedCoords,
  // 1 (gold) + 6 (cloud) + 42 (outer) = 49 hexes needing number tokens
